...
// Single test for background mode - to be called by Python script
//
...
bool testBackground(std::string pmt, int width, std::vector<int64_t> &originsA,
...
int main(int argc, char *argv[]) {
    ...
    if (Comm::isClient()) {
        Log::i("Background benchmark - Primitive: {}, Num: {}, Width: {}", primitive, num, width);
    }
    ...
    if (!testBackground(primitive, width, originsA, originsB, conditions, backgroundTime)) {
        if (Comm::isClient()) {
