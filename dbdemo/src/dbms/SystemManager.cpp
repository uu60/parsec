...
#include "operator/CreateSupport.h"
    ...
    for (int si = 0; si < result.getStatements().size(); si++) {
        ...
        switch (stmt->type()) {
            case hsql::kStmtCreate: {
                if (!CreateSupport::clientCreateTable(resp, stmt)) goto over;
                break;
            }
            case hsql::kStmtDrop: {
                if (!DropSupport::clientDropTable(resp, stmt)) goto over;
                break;
            }
            case hsql::kStmtInsert: {
                if (!InsertSupport::clientInsert(resp, stmt)) goto over;
                break;
            }
            case hsql::kStmtSelect: {
                if (!SelectSupport::clientSelect(resp, stmt)) goto over;
                break;
            }
            default: {
...
void SystemManager::serverExecute() {
    while (true) {
        ...
        switch (commandType) {
            ...
            case CREATE_TABLE: {
                CreateSupport::serverCreateTable(j);
                break;
            }
            case DROP_TABLE: {
                DropSupport::serverDropTable(j);
                break;
            }
            case INSERT: {
                InsertSupport::serverInsert(j);
                break;
            }
            case SELECT: {
                SelectSupport::serverSelect(j);
                break;
            }
            case UNKNOWN: {
