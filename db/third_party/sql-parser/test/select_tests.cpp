...
TEST(CastExpression) {
  TEST_PARSE_SINGLE_SQL("SELECT CAST(10 AS INT(10));", kStmtSelect, SelectStatement, result, stmt);
  ...
  ASSERT_EQ(stmt->selectList->at(0)->columnType, ColumnType(DataType::INT, 10));
  ASSERT_EQ(stmt->selectList->at(0)->expr->type, kExprLiteralInt);
}
