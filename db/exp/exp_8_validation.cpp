...
static std::vector<std::pair<int64_t, int64_t> > computeExpected(
    const std::vector<int64_t> &c_keys,
    const std::vector<int64_t> &o_cust,
    const std::vector<int64_t> &o_flag) {
...
View filterOrdersByComment(View &orders_view, int64_t word, int tid) {
    ...
    View v = orders_view;
    v.filterAndConditions(fields, cmps, consts, tid);
    return v;
}
...
View performLeftOuterJoin(View &customer_view, View &filtered_orders_view, int tid) {
    ...
    View joined_view = Views::leftOuterJoin(customer_view, filtered_orders_view, customer_key, order_key, false, false);

    return joined_view;
}

View groupByCustomerAndCount(View &joined_view, int tid) {
    // Group by c_custkey
    std::vector<std::string> groupFields = {"customer.c_custkey"};
    auto group_boundaries = joined_view.groupBy(groupFields[0], tid);

    std::vector<std::string> countFields = {"orders.o_custkey"};
    // Count orders for each customer group
    joined_view.count(countFields, group_boundaries, "c_count", tid);
    joined_view._fieldNames[0] = "c_custkey";

    return joined_view;
}

View groupByCountAndCountCustomers(View &customer_counts_view, int tid) {
    // Select only c_count column for grouping
    std::vector<std::string> fields = {"c_count"};
    customer_counts_view.select(fields);

    // Group by c_count
    auto group_boundaries = customer_counts_view.groupBy("c_count", tid);

    // Count customers for each c_count value (this gives us custdist)
    customer_counts_view.count(fields, group_boundaries, "custdist", tid);

    return customer_counts_view;
}
...
View sortFinalResults(View &final_view, int tid) {
    ...
    final_view.sort(sortFields, sortOrders, tid);
...
int main(int argc, char *argv[]) {
    ...
    if (Comm::isServer()) {
        ...
        // 左外连接
        auto vJ = performLeftOuterJoin(vC, vOf, tid + 200);
        auto t2 = System::currentTimeMillis();
        Log::i("Left outer join time: {}ms", (t2 - t1));

        // 每客户计数
        auto vCnt = groupByCustomerAndCount(vJ, tid + 300);
        auto t3 = System::currentTimeMillis();
        Log::i("Group by customer and count time: {}ms", (t3 - t2));

        // 直方图
        auto vHist = groupByCountAndCountCustomers(vCnt, tid + 400);
        auto t4 = System::currentTimeMillis();
        Log::i("Group by count and count customers time: {}ms", (t4 - t3));

        // 排序
        auto vOut = sortFinalResults(vHist, tid + 500);
