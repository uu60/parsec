...
int main(int argc, char *argv[]) {
    ...
    if (Comm::isClient()) {
        ...
        Comm::send(r1,            64, 0, tid + 1);
        Comm::send(r1 ^ kASP,     64, 1, tid + 1);
    } else {
        // 每个 server 各收自己的那一份
        Comm::receive(hd_share,  64, 2, tid);
        Comm::receive(asp_share, 64, 2, tid + 1);
    }
    ...
    if (Comm::isServer()) {
        ...
        // 谓词下推
        auto dv = filterEquals(diagnosis_view, col_diag, hd_share,  tid + 10);
        auto mv = filterEquals(medication_view, col_med, asp_share, tid + 20);

        // 每 pid 一行（d: MIN(time)，m: MAX(time)）
        auto d_min = perPidMinOrMax(dv, /*keepMinTime=*/true,  tid + 100);
        auto m_max = perPidMinOrMax(mv, /*keepMinTime=*/false, tid + 200);
        ...
        auto le_bits = build_leq_bits(d_time_col, m_time_col, tid + 300);

        // 应用有效位并压缩
        int valid_idx = joined.colNum() + View::VALID_COL_OFFSET;
        joined._dataCols[valid_idx] = std::move(le_bits);
        joined.clearInvalidEntries(tid + 400);
...
View perPidMinOrMax(View v, bool keepMinTime, int tid) {
    ...
    auto heads = v.groupBy(grp_col, false, tid + 1);

    // heads & $valid 作为新的有效位，仅保留“组首且命中”的代表行
    int vidx = v.colNum() + View::VALID_COL_OFFSET;
    auto hv = BoolAndBatchOperator(&heads, &v._dataCols[vidx], 1, 0, tid + 2,
                                   SecureOperator::NO_CLIENT_COMPUTE).execute()->_zis;
    v._dataCols[vidx] = std::move(hv);

    v.clearInvalidEntries(tid + 3);
    return v;
}
